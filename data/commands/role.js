...
export const roleCommand = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub.setName('add').setDescription('Add a role to a user')
      ...
      .addStringOption((opt) => opt.setName('reason').setDescription('Reason for assigning the role'))
  )
  .addSubcommand((sub) =>
    sub.setName('allow').setDescription('Allow a role to be requested via /rolerequest')
      .addRoleOption((opt) => opt.setName('role').setDescription('The role to allow').setRequired(true))
  )
  .addSubcommand((sub) =>
    sub.setName('disallow').setDescription('Disallow a role from being requested via /rolerequest')
      .addRoleOption((opt) => opt.setName('role').setDescription('The role to disallow').setRequired(true))
  );
...
export async function handleRole(interaction) {
  ...
  if (sub === 'add') {
    ...
    if (!config.roleChannel) {
      return interaction.reply({
        content: '❌ No role channel set. Use `/rolechannel set` first.',
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      await interaction.editReply({ content: '❌ Failed to assign the role. Check bot permissions and role hierarchy.' });
    }
    return;
  }
  ...
  if (sub === 'allow') {
    ...
    return interaction.reply({ content: `✅ **${role.name}** is now **allowed** for \`/rolerequest\`.`, ephemeral: true });
  }
  ...
  if (sub === 'disallow') {
    ...
    return interaction.reply({ content: `✅ **${role.name}** is now **disallowed** from \`/rolerequest\`.`, ephemeral: true });
  }
}
