...
export const loaRequestCommand = new SlashCommandBuilder()
  ...
  .addStringOption((opt) => opt.setName('length').setDescription('How long will your LOA last? (e.g. 1 week, 3 days)').setRequired(true))
  .addStringOption((opt) => opt.setName('notes').setDescription('Any additional notes'));

export async function handleLoaRequest(interaction) {
  ...
  if (!config.loaChannel) {
    return interaction.reply({ content: '❌ No LOA channel set. Use `/loachannel` first.', ephemeral: true });
  }

  const loaChannel = interaction.guild.channels.cache.get(config.loaChannel);
  if (!loaChannel) {
    return interaction.reply({ content: '❌ The configured LOA channel no longer exists.', ephemeral: true });
  }
  ...
  try {
    await submitter.roles.add(LOA_ROLE_ID, 'LOA request submitted');
  } catch {
    return interaction.editReply({ content: '❌ Failed to assign the LOA role. Please contact an administrator.' });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0xffa500)
    .addFields(
      { name: 'Member',      value: `${submitter} | ${submitter.displayName}`, inline: true },
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`loa_approve_${interaction.user.id}`).setLabel('Approve').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`loa_deny_${interaction.user.id}`).setLabel('Deny').setStyle(ButtonStyle.Danger)
  );
...
export async function handleLoaButton(interaction) {
  ...
  if (!isAdmin && !reviewer.roles.cache.has(PRIVILEGED_ROLE_ID)) {
    return interaction.reply({ content: '❌ You do not have permission to approve or deny LOA requests.', ephemeral: true });
  }
  ...
  const resultRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setLabel(`LOA ${isApprove ? 'Approved' : 'Denied'} By: ${reviewer.displayName}`)
